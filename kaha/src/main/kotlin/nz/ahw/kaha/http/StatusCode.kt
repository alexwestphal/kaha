...
**                                             **
\*---------------------------------------------*/
package nz.ahw.kaha.http

abstract class StatusCode protected constructor() {
    ...
    abstract fun isRedirection(): Boolean
}

object StatusCodes {

    abstract class HttpSuccess protected constructor(): StatusCode() {
        override fun isSuccess(): Boolean = true
        override fun isFailure(): Boolean = false
    }

    abstract class HttpFailure protected constructor(): StatusCode() {
        override fun isSuccess(): Boolean = false
        override fun isFailure(): Boolean = true
        override fun isRedirection(): Boolean = false
    }

    data class Informational(override val value: Int, override val reason: String, override val defaultMessage: String): HttpSuccess() {
        override fun isRedirection(): Boolean = false
    }

    data class Success(override val value: Int, override val reason: String, override val defaultMessage: String): HttpSuccess() {
        override fun isRedirection(): Boolean = false
    }

    data class Redirection(override val value: Int, override val reason: String, override val defaultMessage: String): HttpSuccess() {
        override fun isRedirection(): Boolean = true
    }

    data class ClientError(override val value: Int, override val reason: String, override val defaultMessage: String): HttpFailure()
    data class ServerError(override val value: Int, override val reason: String, override val defaultMessage: String): HttpFailure()

    val Continue           = Informational(100,"Continue", "The server has received the request headers, and the client should proceed to send the request body.")
    val SwitchingProtocols = Informational(101,"Switching Protocols", "The server is switching protocols, because the client requested the switch.")
    val Processing         = Informational(102,"Processing", "The server is processing the request, but no response is available yet.")
    ...
    val NoContent                   = Success(204,"No Content", "The server successfully processed the request and is not returning any content.")
    ...
    val MultipleChoices   = Redirection(300,"Multiple Choices", "There are multiple options for the resource that the client may follow.")
    val MovedPermanently  = Redirection(301,"Moved Permanently", "This and all future requests should be directed to the given URI.")
    val Found             = Redirection(302,"Found", "The resource was found, but at a different URI.")
    val SeeOther          = Redirection(303,"See Other", "The response to the request can be found under another URI using a GET method.")
    val NotModified       = Redirection(304,"Not Modified", "The resource has not been modified since last requested.")
    val UseProxy          = Redirection(305,"Use Proxy", "This single request is to be repeated via the proxy given by the Location field.")
    val TemporaryRedirect = Redirection(307,"Temporary Redirect", "The request should be repeated with another URI, but future requests can still use the original URI.")
    val PermanentRedirect = Redirection(308,"Permanent Redirect", "The request, and all future requests should be repeated using another URI.")
    ...
    val ImATeapot                    = ClientError(418,"I'm a teapot", "The resulting entity body MAY be short and stout.")
    val EnhanceYourCalm              = ClientError(420,"Enhance Your Calm", "You are being rate-limited.")
    val MisdirectedRequest           = ClientError(421,"Misdirected Request", "The request was directed at a server that is not able to produce a response.")
    ...
    val GatewayTimeout                = ServerError(504,"Gateway Timeout", "The server was acting as a gateway or proxy and did not receive a timely response from the upstream server.")
